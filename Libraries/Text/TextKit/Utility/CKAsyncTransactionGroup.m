...
+ (void)registerTransactionGroupAsMainRunloopObserver:(CKAsyncTransactionGroup *)transactionGroup
{
  ...
  RCTAssert(observer == NULL, @"A CKAsyncTransactionGroup should not be registered on the main runloop twice");
...
{
  RCTAssertMainThread();
  RCTAssert(containerLayer != nil, @"Cannot add a nil layer to the group");
...
{
  RCTAssertMainThread();
  RCTAssert(containerLayer != nil, @"Cannot remove a nil layer from the group");
...
{
  RCTAssertMainThread();
  RCTAssert(completionHandler != NULL, @"Calling this method without a completion handler makes no sense");
