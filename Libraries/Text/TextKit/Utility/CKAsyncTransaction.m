...
- (void)dealloc
{
  RCTAssert(_operationCompletionBlock == nil, @"Should have been called and released before -dealloc");
}
...
- (void)dealloc
{
  ...
  RCTAssert(_state != CKAsyncTransactionStateOpen, @"Uncommitted CKAsyncTransactions are not allowed");
}
...
{
  RCTAssertMainThread();
  RCTAssert(_state == CKAsyncTransactionStateOpen, @"You can only add operations to open transactions");
...
{
  RCTAssertMainThread();
  RCTAssert(_state != CKAsyncTransactionStateOpen, @"You can only cancel a committed or already-canceled transaction");
...
{
  RCTAssertMainThread();
  RCTAssert(_state == CKAsyncTransactionStateOpen, @"You cannot double-commit a transaction");
  ...
  if ([_operations count] == 0) {
    ...
    RCTAssert(_group != NULL, @"If there are operations, dispatch group should have been created");
